    ...
    def run(self):
        ...
        while running:
            ...
            if command == 0b10000010:  # LDI
                num = self.ram[self.pc + 1]  # grabs 1st arg and set as num
                reg = self.ram[self.pc + 2]  # register index is the 2nd arg

                self.register[num] = reg  # store the num into register
                self.pc += 3

            elif command == 0b01000111:  # PRN
                reg = self.ram[self.pc + 1]  # register index is 1st arg
                print(self.register[reg])  # print
                self.pc += 2
